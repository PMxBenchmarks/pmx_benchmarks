...
If any data file exceeds 100 MB, consider using dvs for blob storage instead of
git-lfs (required for files > 2 GB). See `scripts/setup_dvs.R` for setup and
`dvs.yaml` for backend configuration. Contact the maintainers for storage credentials.

| File size | Method |
|---|---|
| < 100 MB | git-lfs (handled automatically) |
| 100 MB – 2 GB | git-lfs, or dvs if bandwidth is a concern |
| > 2 GB | dvs required — contact maintainers |
...
```yaml
# Soft predictions (required for rank-based metrics like auroc)
- name: dropout-prediction
  type: classification
  output_format: {type: probabilities, columns: [ID, P_DROPOUT]}
  metric: auroc

# Hard predictions (use with accuracy, f1)
...
from your generative model. Its `estimates` keys must exactly match the
values declared in `output_format`:
...
Other supported counterfactual output types: `summary_stats` (with `stats: [mean, sd, ...]`)
and `probability` (with `threshold` and `direction: above|below`; truth file needs `p` key).
